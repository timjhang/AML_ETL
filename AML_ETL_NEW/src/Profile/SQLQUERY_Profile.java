...
public class SQLQUERY_Profile {
	...
	private static String SQLQUERY2 = "SELECT DISTINCT A.ETL_CREATE_DATE AS 檔案日期, A.FILE_TYPE AS 業務別, A.DOMAIN_ID AS 本會代號, SUBSTRING(B.PARTY_KEY,9,11) AS 客戶統編, SUBSTRING(A.ACCOUNT_KEY,9,30) AS 帳號, A.ACCOUNT_TYPE_CODE AS 帳戶類別, A.STATUS_CODE AS 帳戶狀態, D.BRANCH_CODE AS 帳戶行 FROM SRC.ACCOUNT A LEFT JOIN SRC.PARTY_ACCOUNT_REL B  ON A.ACCOUNT_KEY = B.ACCOUNT_KEY  AND A.RECORD_DATE = B.RECORD_DATE LEFT JOIN SRC.ACCOUNT D  ON A.ACCOUNT_KEY = D.ACCOUNT_KEY  AND A.RECORD_DATE = D.RECORD_DATE WHERE NOT EXISTS (SELECT 1 FROM SRC.PARTY C WHERE C.PARTY_KEY = B.PARTY_KEY AND C.RECORD_DATE = B.RECORD_DATE) AND A.DATA_SOURCE_FILE_NAME IN ('SELF') AND A.RECORD_DATE = ? ORDER BY D.BRANCH_CODE, A.FILE_TYPE, A.DOMAIN_ID, SUBSTRING(B.PARTY_KEY,9,11), SUBSTRING(A.ACCOUNT_KEY,9,30)";
	...
	private static String SQLQUERY4 = "SELECT DISTINCT A.ETL_CREATE_DATE AS 檔案日期, A.FILE_TYPE AS 業務別, A.DOMAIN_ID AS 本會代號, SUBSTRING(A.PARTY_KEY,9,11) AS 客戶統編, SUBSTRING(A.LOAN_KEY,9,30) AS 放款帳號, SUBSTRING(A.LOAN_DETAIL_KEY,9,20) AS 額度編號, A.LOAN_DATE AS 初貸日, A.LOAN_STATUS_CODE AS 放款狀態, A.EXECUTION_BRANCH_CODE AS 帳戶行 FROM SRC.LOAN A WHERE NOT EXISTS (SELECT 1 FROM SRC.LOAN_DETAIL B WHERE A.LOAN_DETAIL_KEY = B.LOAN_DETAIL_KEY AND A.RECORD_DATE = B.RECORD_DATE) AND A.RECORD_DATE = ? ORDER BY A.FILE_TYPE, A.DOMAIN_ID, A.EXECUTION_BRANCH_CODE";
	...
	public static String[] getEmptyHead(String currentQuery) {
		...
		String[] sqlquery2 = { "檔案日期", "業務別", "本會代號", "客戶統編", "帳號", "帳戶類別", "帳戶狀態","帳戶行" };
		String[] sqlquery3 = { "檔案日期", "業務別", "本會代號", "客戶統編", "額度編號", "額度核准日", "批覆書編號", "批覆書申請日" };
		String[] sqlquery4 = { "檔案日期", "業務別", "本會代號", "客戶統編", "放款帳號", "額度編號", "初貸日", "放款狀態","帳戶行" };
