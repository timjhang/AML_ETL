...
public class ETL_T_PARTY_ADDRESS extends Transform {
	...
	public ETL_T_PARTY_ADDRESS(ETL_Bean_LogData logData) {
		super(logData);
	}
	...
	public void trans_to_PARTY_ADDRESS_LOAD(ETL_Bean_LogData logData) {
		...
		System.out.println("#######Transform - ETL_T_PARTY_ADDRESS - Start");

		try {

			String sql = "{call " + ETL_Profile.db2TableSchema + ".Transform.TempTo_PARTY_ADDRESS_LOAD(?,?,?)}";
		...
		System.out.println("#######Transform - ETL_T_PARTY_ADDRESS - End");
	}

	public static void main(String[] argv) {

		ETL_T_PARTY_ADDRESS one = new ETL_T_PARTY_ADDRESS();
		...
		one.trans_to_PARTY_ADDRESS_LOAD(two);
	}
}
