...
public class ETL_T_CALENDAR_LOAD extends Transform {
	
	public ETL_T_CALENDAR_LOAD() {
		
	}

	public ETL_T_CALENDAR_LOAD(ETL_Bean_LogData logData) {
		super(logData);
	}
	...
	public void trans_File() {
		try {
			trans_to_CALENDAR_LOAD(this.logData);
		} catch (Exception e) {
			e.printStackTrace();
		} 
	}
	
	// 觸發DB2轉換Procedure, 轉換資料寫進CALENDAR_LOAD
	public void trans_to_CALENDAR_LOAD(ETL_Bean_LogData logData) {
		...
		System.out.println("#######Transform - ETL_T_CALENDAR_LOAD - End");
